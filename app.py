...
def stage1_process():
    ...
    with col2:
        ...
        st.info("複数ファイルを選択可能です（過年度分を含む場合）")
...
def process_stage2(excel_file):
    ...
    # 取引先候補1 → 貸方取引先
    if '取引先候補1' in df.columns and '貸方取引先' in df.columns:
        mask = (df['取引先候補1'].notna()) & (df['取引先候補1'] != '')
        df.loc[mask, '貸方取引先'] = df.loc[mask, '取引先候補1']
    ...
    # 部門候補1 → 借方部門・貸方部門
    if '部門候補1' in df.columns:
        mask = (df['部門候補1'].notna()) & (df['部門候補1'] != '')
        if '借方部門' in df.columns:
            df.loc[mask, '借方部門'] = df.loc[mask, '部門候補1']
        if '貸方部門' in df.columns:
            df.loc[mask, '貸方部門'] = df.loc[mask, '部門候補1']
